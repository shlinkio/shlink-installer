...
class ShlinkAssetsHandlerTest extends TestCase
{
    ...
    public function cachedConfigIsDeletedIfExists(bool $appExists, bool $routesExist, int $expectedRemoveCalls): void
    {
        ...
        $this->filesystem
            ->expects($this->exactly($expectedRemoveCalls))
            ->method('remove')
            ->with(
    ...
    public function errorWhileDeletingCachedConfigIsPropagated(): void
    {
        $this->filesystem
            ->expects($this->once())
            ->method('exists')
            ->with('data/cache/app_config.php')
            ->willReturn(
                true,
            );
        $this->filesystem
    ...
    public function configIsImportedOnlyIfExistingPathIsProvided(bool $exists, mixed $_): void
    {
        ...
        $this->io
            ->expects($this->exactly($exists ? 1 : 4))
            ->method('confirm')
            ->willReturnCallback(
                static function (string $argument) use (&$count) {
        ...
        $this->io
            ->expects($this->exactly($exists ? 1 : 3))
            ->method('ask')
            ->willReturn($importPath);
        $this->filesystem
            ->expects($this->exactly($exists ? 1 : 3))
            ->method('exists')
            ->with(
    ...
    public function assetsAreProperlyImportedIfTheyExist(bool $assetsExist, InvocationOrder $expectedCopies): void
    {
        ...
        $this->filesystem
            ...
            ->willReturnMap(array_map(
                static fn (string $asset) => [$path . '/data/' . $asset, $assetsExist],
                $assets,
            ));
