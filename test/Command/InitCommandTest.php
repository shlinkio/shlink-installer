...
class InitCommandTest extends TestCase
{
    ...
    public function expectedCommandsAreRunBasedOnInput(array $input, array $commands, bool $interactive): void
    {
        $this->commandsRunner
            ...
            ->method('execPhpCommand')
            ->willReturnCallback(
                static function (string $commandName, $_, bool $isInteractive, array $args) use (
                    $commands,
                    $interactive,
                ): bool {
                    ...
                    Assert::assertEquals(
                        $commandName === InstallationCommand::API_KEY_CREATE->value ? ['foo'] : [],
                        $args,
                    );
    ...
    public static function provideInputs(): iterable
    {
        ...
        yield 'skips' => [
            ['--skip-initialize-db' => true, '--skip-download-geolite' => true],
            [
                InstallationCommand::DB_MIGRATE->value,
                InstallationCommand::ORM_PROXIES->value,
            ],
